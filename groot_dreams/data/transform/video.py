...
class VideoCrop(VideoTransform):
    target_height: int = Field(default=480, description="Target height used to determine crop aspect ratio")
    target_width: int = Field(default=640, description="Target width used to determine crop aspect ratio")
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        ...
        if self.backend == "torchvision":
            ...
            return lambda frames: self.__class__.center_crop(frames, target_ratio)
        else:
            raise ValueError(f"Backend {self.backend} not supported")
    ...
    @staticmethod
    def center_crop(frames: torch.Tensor, target_ratio: float = 640 / 480) -> torch.Tensor:
