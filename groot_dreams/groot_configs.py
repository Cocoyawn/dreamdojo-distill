...
def construct_modality_config_and_transforms(
    ...
    elif embodiment == "piper":
        timestep_interval = 3
        delta_indices = list(range(0, num_frames * timestep_interval, timestep_interval))
        video_key = video_key_override or "video.cam_vertical"
        config = {
            "video": ModalityConfig(
                delta_indices=delta_indices,
                modality_keys=[video_key],
    ...
    train_transform = ComposedModalityTransform(
        transforms=[
            ...
            VideoCrop(apply_to=video_modality.modality_keys, target_height=height, target_width=width),
