...
# 720x320 variant — generated by the new teacher (Shirk6/dreamdojo-piper-insert-mouse-battery-720-320-10fps-40k).
dataset_gr00t_piper_720_320_warmup = L(ActionDatasetSFWarmup)(
    data_path="datasets/piper_720_320_warmup_regenerated_4step",
    cr1_embeddings_path="datasets/cr1_empty_string_text_embeddings.pt",
    fps=10,
)
...
def register_interactive_data():
    ...
    for split in ["train", "val"]:
        ...
        cs.store(
            group=f"data_{split}",
            package=f"dataloader_{split}",
            name="gr00t_piper_720_320_warmup",
            node=L(make_dataloader)(dataset=dataset_gr00t_piper_720_320_warmup),
        )
        cs.store(
            group=f"data_{split}",
            package=f"dataloader_{split}",
        ...
        cs.store(
            ...
            node=L(make_dataloader)(dataset=gr00t_customized_piper_720_320_dataset, num_workers=0, pin_memory=False),
        )
        cs.store(
            group=f"data_{split}",
            package=f"dataloader_{split}",
            name="gr00t_customized_piper_720_320_long",
            node=L(make_dataloader)(dataset=gr00t_customized_piper_720_320_dataset_long, num_workers=0, pin_memory=False),
        )
